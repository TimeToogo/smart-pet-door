...
class Annotator:
    ...
    def process_keyboard_input(self, key_input, run):
        ...
        if chr(key_input) in {chr(d) for d in range(ord('0'),ord('9')+1)}:
            self.label_key_buff.append(chr(key_input))
        
        if chr(key_input) in {'l', 'L'}:
            if len(self.label_key_buff) == 0:
                print('type label number first')
            else:
                label = int(''.join(self.label_key_buff))
                if label > len(self.labels):
                    print('Error: label %s not implemented' % str(label))
                else:
                    self.selected_label = label-1
